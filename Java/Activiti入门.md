...
1. act_re_ *: 这些表包含了流程定义和流程静态资源，如版本，发布名称等。
2. act_ru_ *: 这些运行时的表，包含流程实例，任务，变量，异步任务，等运行中的数据。 Activiti只在流程实例执行过程中保存这些数据，在流程或任务结束时就会删除这些记录。 
3. act_id_ *:  这些表包含身份信息，如用户，组等。
4. act_hi_ *: 这些表包含历史数据，如历史流程实例，变量，任务等，正在运行的任务和流程也会保存。
5. act_ge_ *: 通用数据，如存放资源文件，流程保存的自定义类型数据等。
